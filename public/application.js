...
/***/ (function(module, exports, __webpack_require__) {
	...
	var surveyEvents = __webpack_require__(37);
	$(function () {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var store = __webpack_require__(9);
	var surveysUI = __webpack_require__(12);
	var surveysAPI = __webpack_require__(34);
	...
	var signInSuccess = function signInSuccess(data) {
	  ...
	  surveysAPI.onGetSurveys().then(surveysUI.getSurveysSuccess).catch(surveysUI.getSurveysFailure);
	...
	var showModalMessage = function showModalMessage(message) {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var store = __webpack_require__(9);
	var showSurveysTemplate = __webpack_require__(13);
	var showQuestionsTemplate = __webpack_require__(33);
	var api = __webpack_require__(34);
	var showDashboardTemplate = __webpack_require__(35);
	var showAllSurveysTemplate = __webpack_require__(36);
	...
	var getSurveysSuccess = function getSurveysSuccess(data) {
	  ...
	  for (var i = 0; i < data.surveys.length; i++) {
	    var item = data.surveys[i];
	    if (item.editable) {
	      surveys.push(data.surveys[i]);
	    }
	  }
	  var onQuestionSubmit = function onQuestionSubmit(event) {
	    event.preventDefault();
	  ...
	  // })
	  var showSurveysHtml = showSurveysTemplate({ surveys: surveys });
	  $('#userSurveys').empty();
	  ...
	  $('.delete-survey').on('click', function (event) {
	    ...
	    api.onDeleteSurvey(data[2]).then(onDeleteSurveySuccess).catch(onDeleteSurveyFailure);
	  });
	  $('.update-survey').on('click', onUserUpdateSurvey);
	  $('.take-survey').on('click', onUserTakeSurvey);
	  // show all surveys
	  var showAllSurveys = showAllSurveysTemplate({ surveys: store.allSurveys });
	  $('#allTakeSurveys').empty();
	...
	var onSurveyDashboard = function onSurveyDashboard() {
	  ...
	  api.onGetSurveyQuestions(surveyData).then(function (questionsData) {
	    console.log('onGetSurveyQuestions databack is ', questionsData);
	    return questionsData;
	  }).then(function (questionsData) {
	    api.onGetSingleSurvey(store.dashboardID).then(function (databack) {
	      console.log('databack is ', databack);
	      var showDashboardHtml = showDashboardTemplate({ survey: databack.survey, questions: questionsData.questions });
	...
	var getSurveysFailure = function getSurveysFailure(error) {
	  console.log('Get Surveys Failure');
	  console.error(error);
	};
	var onCreateSurveySuccess = function onCreateSurveySuccess(data) {
	  ...
	  // store.surveys = data.surveys
	  api.onGetSurveys().then(getSurveysSuccess).catch(getSurveysFailure);
	};
	var onCreateSurveyFailure = function onCreateSurveyFailure(error) {
	  console.log('Get Surveys Failure');
	  console.error(error);
	};
	var onDeleteSurveySuccess = function onDeleteSurveySuccess(data) {
	  // update the store with the vaild surveys
	  api.onGetSurveys().then(getSurveysSuccess).catch(getSurveysFailure);
	  console.log('Delete Survey success', data);
	  // store.surveys = data.surveys
	};
	var onDeleteSurveyFailure = function onDeleteSurveyFailure(error) {
	  console.log('Delete Surveys Failure');
	  console.error(error);
	};
	var onUpdateSurveySuccess = function onUpdateSurveySuccess() {
	  // update the store with the vaild surveys
	  console.log('Update Survey success');
	  // update the store with the vaild surveys
	  api.onGetSurveys().then(getSurveysSuccess).catch(getSurveysFailure);
	};
	var onUpdateSurveyFailure = function onUpdateSurveyFailure(error) {
	  console.log('Update Surveys Failure');
	  console.error(error);
	};

	var onCreateQuestionSuccess = function onCreateQuestionSuccess(data) {
	  console.log('Create Question success', data);
	  // store.surveys = data.surveys
	};
	var onCreateQuestionFailure = function onCreateQuestionFailure(error) {
	  console.log('create-survey Question Failure');
	  console.error(error);
	};
	var onDeleteQuestionSuccess = function onDeleteQuestionSuccess() {
	  console.log('Delete Question success');
	  // store.surveys = data.surveys
	};
	var onDeleteQuestionFailure = function onDeleteQuestionFailure(error) {
	  console.log('Delete Question Failure');
	  console.error(error);
	};
	var onUserUpdateSurvey = function onUserUpdateSurvey(event) {
	  ...
	  var name = findSurveyNameByID(data[2]);
	  console.log('survey current name: ' + name);
	  $('#current-survey-name').val(name);
	  $('#surveyUpdateModal').modal('show');
	};
	var findSurveyNameByID = function findSurveyNameByID(id) {
	  for (var i = 0; i < store.surveys.length; i++) {
	    var item = store.surveys[i];
	    if (item.id === id) {
	      return item.name;
	    }
	  }
	};
	var onUserTakeSurvey = function onUserTakeSurvey(event) {
	  ...
	  api.onGetSurveyQuestions(surveyID).then(onGetSurveyQuestionsSuccess).catch(onGetSurveyQuestionsFailure);
	};
	var onGetSurveyQuestionsFailure = function onGetSurveyQuestionsFailure(error) {
	  console.log('onGetSurveyQuestionsFailure Failure');
	  console.error(error);
	};
	var onGetSurveyQuestionsSuccess = function onGetSurveyQuestionsSuccess(data) {
	  console.log('onGetSurveyQuestionsSuccess success', data);
	  $('#takeSurvey').empty();
	  store.questions = data.questions;
	  var showQuestions = showQuestionsTemplate({ questions: data.questions });
	...
	var onQuestionSubmit = function onQuestionSubmit(event) {
	  ...
	  var id = data[1];
	  console.log('id: ', id);
	  var questionAnswer = $('#questionInput' + id).val();
	  console.log('questionAnswer: ' + questionAnswer);
	  var sumOfAnswers = 0;
	  var numberOfTimesAnswered = 0;
	  for (var i = 0; i < store.questions.length; i++) {
	    var item = store.questions[i];
	    if (item.id === id) {
	      sumOfAnswers = item.sumOfAnswers;
	  ...
	  var questionUpdateData = {
	    'question': {
	      ...
	      'numberOfTimesAnswered': numberOfTimesAnswered
	    }
	  };
	  api.onUpdateQuestion(questionUpdateData, id).then(console.log);
	};
	module.exports = {
	  getSurveysSuccess: getSurveysSuccess,
	  getSurveysFailure: getSurveysFailure,
	  onCreateSurveySuccess: onCreateSurveySuccess,
	  onCreateSurveyFailure: onCreateSurveyFailure,
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
	  ...
	  return "  <div class='survey-"
	    ...
	    + " survey'>\n    <h1><a href='#survey-dashboard' id=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\" class=\"survey-dashboard-link lead\">"
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.name : stack1), depth0))
	    + "</a></h1>\n    <input type='button' value='Delete' id='survey-delete-"
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "' class='delete-survey'>\n  </div>\n  <hr>\n";
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
	  ...
	  return "  <div class='question"
	    ...
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "' placeholder=\"1-5\" min=\"1\" max=\"5\"></ul>\n    <input type='button' value='Submit' id='question-"
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "' class='question-survey-button'>\n";
	},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
	    var stack1;
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var onGetSingleSurvey = function onGetSingleSurvey(id) {
	...
	var onGetSurveyQuestions = function onGetSurveyQuestions(data) {
	...
	var onCreateSurvey = function onCreateSurvey(dataIn) {
	...
	var onUpdateSurvey = function onUpdateSurvey(data, id) {
	...
	var onCreateQuestion = function onCreateQuestion(question, questionNum, surveyID) {
	  return $.ajax({
	...
	var onDeleteQuestion = function onDeleteQuestion(data) {
	...
	var onUpdateQuestion = function onUpdateQuestion(data, id) {
	...
	module.exports = {
	  ...
	  onGetSurveyQuestions: onGetSurveyQuestions,
	  onCreateQuestion: onCreateQuestion,
	  onDeleteQuestion: onDeleteQuestion,
	  onGetSingleSurvey: onGetSingleSurvey,
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
	  ...
	  return "   <ul class=\"question-dashboard-header\" id=\"question-"
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\">"
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.question : stack1), depth0))
	    + "<br/>\n     <li class=\"question-stat\">Total Answers are "
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.numberOfTimesAnswered : stack1), depth0))
	    + "<br/>\n     <li class=\"question-stat\">Average Score for this question is "
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.avgScore : stack1), depth0))
	    + "\n </ul>\n";
	},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
	    var stack1, alias1=container.lambda, alias2=container.escapeExpression;

	  return "<h1 id=\"survey-name-dashboard\"><h2><data-id=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.survey : depth0)) != null ? stack1.id : stack1), depth0))
	    + "\">"
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.survey : depth0)) != null ? stack1.name : stack1), depth0))
	    + "</h1>\n\n"
	    + ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.questions : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "");
	},"useData":true,"useBlockParams":true});
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
	  ...
	  return "  <div class='survey-"
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + " survey'>\n    <h1 class=\"survey-dashboard-link\">"
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.name : stack1), depth0))
	    + "</a></h1>\n    <input type='button' value='Take Survey' id='survey-take-"
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "' class='take-survey'>\n  </div>\n  <hr>\n";
	},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
	    var stack1;
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var onGetSurveys = function onGetSurveys(event) {
	  ...
	  api.onGetSurveys().then(ui.getSurveysSuccess).catch(ui.getSurveysFailure);
	};
	var onCreateSurvey = function onCreateSurvey(event) {
	  event.preventDefault();
	  console.log('create survey');
	  var data = $('#survey-create-input').val();
	  console.log('Data is: ', data);
	  api.onCreateSurvey(data).then(ui.onCreateSurveySuccess).catch(ui.onCreateSurveyFailure);
	};

	var onUpdateSurvey = function onUpdateSurvey(event) {
	  event.preventDefault();
	  var data = getFormFields(event.target);
	  api.onUpdateSurvey(data, store.updateSurveyID).then(ui.onUpdateSurveySuccess).catch(ui.onUpdateSurveyFailure);
	};
	var onDeleteSurvey = function onDeleteSurvey(event) {
	  ...
	  var data = event.target.id.split('-');
	  console.log('id: ', data[2]);
	  api.onDeleteSurvey(data[2]).then(ui.onDeleteSurveySuccess).catch(ui.onDeleteSurveyFailure);
	};
	var onCreateQuestion = function onCreateQuestion(event) {
	  ...
	  for (var prop in data.questionSchema) {
	    questionArray.push(data.questionSchema[prop]);
	  }
	  console.log(questionArray);
	  // api.onCreateQuestion()
	  questionArray.forEach(function (e, i, a) {
	    var questionNum = i + 1;
	    api.onCreateQuestion(e, questionNum, store.surveyID).then(ui.onCreateQuestionSuccess).catch(ui.onCreateQuestionFailure);
	  });
	  $('#create-questions-modal').modal('toggle');
	};
	var onDeleteQuestion = function onDeleteQuestion(event) {
	  event.preventDefault();
	  var data = getFormFields(event.target);
	  api.onDeleteQuestion(data).then(ui.onDeleteQuestionSuccess).catch(ui.onDeleteQuestionFailure);
	};
	var onUpdateSurveyModal = function onUpdateSurveyModal(event) {
	  event.preventDefault();
	...
	var addSurveyHandlers = function addSurveyHandlers() {
	...
	module.exports = {
	  addSurveyHandlers: addSurveyHandlers
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
